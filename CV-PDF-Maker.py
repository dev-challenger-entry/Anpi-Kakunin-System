...
CV-PDF-Maker.py
-------------------
履歴書をPythonで生成するスクリプト。
VSCode上でこの1ファイルを実行するだけで、同じフォルダに「■■■■履歴書■■■■.pdf」が出力される。

■ 使い方
1. このファイルと同じフォルダに、証明写真を "photo.jpg" という名前で置く。
2. 下の「編集用データ」セクションの中身（氏名・住所・電話番号・メールアドレスなど）を
   自分の内容に書き換える。
3. VSCodeでこのファイルを実行する（右上の実行ボタン、または F5、または
   ターミナルで  python CV-PDF-Maker.py  ）。
4. 同じフォルダに ■■■■履歴書■■■■.pdf が生成される。

■ 必要なライブラリ
    pip install reportlab pillow
（日本語フォントは reportlab に内蔵されている CID フォントを使うので、
  別途フォントファイルをダウンロードする必要はない）
"""
